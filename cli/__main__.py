...
def get_permissions(
    ...
    r.raise_for_status()
    liveboards = pd.json_normalize(
        ts.pinboard_list().json(), record_path='headers')
    ...
    teststring = {'PINBOARD_ANSWER_BOOK': liveboard_list,
                  'QUESTION_ANSWER_BOOK': answerlist}
...
def sync_users(
    ctx: typer.Context,
    cfg_name: str = typer.Option(..., help="Name of config file"),
    source_ts_url: str = typer.Option(...,
                                      help="URL of your ThoughtSpot cluster"),
    source_username: str = typer.Option(...,
                                        help="username of your source cluster"),
    source_password: str = typer.Option(...,
                                        help="password of your source cluster"),
    dest_ts_url: str = typer.Option(...,
                                    help="URL of your destination cluster"),
    dest_username: str = typer.Option(...,
                                      help="username of your destination cluster"),
    dest_password: str = typer.Option(...,
                                      help="password of your destination cluster"),
    data_dir: pathlib.Path = typer.Option(...,
                                          help="directory to store output data in"),
):
    """
    ...
    if sync_type == 'all':
        users = pd.read_csv(
            data_dir + "/all_objects/falcon_users.csv", delimiter='|')
    elif sync_type == 'created':
        users = pd.read_csv(
            data_dir + "/created/users_created.csv", delimiter='|')
    else:
        pass
        console.print(
            "wrong parameter specified for sync-type! Choose either all or created", style='warning')
    ...
    t = Texttable()
    try:
        users.rename(columns={"header.name": "name",
                     "header.displayName": "displayName"}, inplace=True)
    except:
        pass

    empty_table_list = [
        ['index', 'username', 'display name', 'assigned groups']]
    for usr in track(range(len(users)), description=f'[bold green]Synchronising {sync_type} users'):
        ...
        assigned_groups = pd.json_normalize(
            ts.users(user_name=username).json())
        ass_grp = assigned_groups['assignedGroups'][0]
        dis_name = assigned_groups['displayName'][0]
        if validate_only == 'no':
            ctu = ps.create_user(
                UserName=username, DisplayName=dis_name, group_guids=ass_grp)
...
def remove_summary(
    ...
    object_type: str = typer.Option(...,
                                    help="valid options: [Answers,Liveboards]"),
):
    """
    ...
    try:
        ...
        console.print("successfully logged in to " +
                      dest_ts_url, style="success")
    ...
    if object_type == 'Answers':
        ...
        # Upload
        Answers = pd.read_csv(
            f"{data_dir}/all_objects/Answer.csv", header=[0], delimiter='|')
        Answers = Answers['id']

        for answer in track(Answers, description='[cyan][bold]Removing headline aggregations for {} Answers'.format(len(Answers))):
            GUID = answer
            try:
                tml = ps.metadata_tml_export(
                    guid=GUID, export_fqn=True, export_associated=False)
                Answer.loads(json.dumps(tml)).dump(
                    f"{data_dir}/tml_export/original/Answer/{GUID}.answer.tml")
            ...
            try:
                UploadObject = ps.metadata_tml_import(
                    tml, create_new_on_server=False, validate_only=validate_flag)
            except Exception as e:
                logging.info(str(e))
                pass
            logging.info(str(GUID)+' --> ' +
                         str(UploadObject['object'][0]['response']['status']))
        output_message("Removed {} headline_aggregations from {}".format(
            len(Answers), dest_ts_url), 'success')

    elif object_type == 'Liveboards':
        Liveboards = pd.read_csv(
            f"{data_dir}/all_objects/Liveboard.csv", header=[0], delimiter='|')
        ...
        for Liveboard_guid in track(Liveboards, description='[cyan][bold]Removing headline aggregations for {} Liveboards'.format(len(Liveboards))):
            ...
            try:
                tml = ps.metadata_tml_export(
                    guid=GUID, export_fqn=True, export_associated=False)
                Liveboard.loads(json.dumps(tml)).dump(
                    f"{data_dir}/tml_export/original/Liveboard/{GUID}.liveboard.tml")
            except Exception as e:
                logging.info(str(e))
                pass
            try:
                ...
                logging.info(
                    "This liveboard seems to be empty and will be skipped")
                numberOfViz = 1
            for i in range(numberOfViz):
                try:
                    numberOfTableColumns = len(
                        tml['liveboard']['visualizations'][i]['answer']['table']['table_columns'])
                except:
            ...
            try:
                UploadObject = ps.metadata_tml_import(
                    tml, create_new_on_server=False, validate_only=validate_flag)
                logging.info(
                    str(GUID)+' --> '+str(UploadObject['object'][0]['response']['status']))
            except Exception as e:
                pass
                logging.info(str(e)+' --> '+GUID)
        output_message("Removed {} headline_aggregations from {}".format(
            len(Liveboards), dest_ts_url), 'success')
    else:
        pass
        """
...
def rollback_tml(
    ...
    console.print(
        f"Uploading original {object_type} tml files", style="bold blue")
    directory = f"{data_dir}/tml_export/original/{object_type}/"
    for filename in os.listdir(directory):
        obj = determine_tml_type(
            path=f"{data_dir}/tml_export/original/{object_type}/{filename}")
        tml = obj.load(
            f"{data_dir}/tml_export/original/{object_type}/{filename}")
        tml_name = tml.name
        data = tml.dumps(format_type="JSON")
        try:
            UploadObject = ps.metadata_tml_import(
                data, create_new_on_server=False, validate_only=validate_flag)
            console.print(
                f"[bold green]successfully[/bold green] recovered [bold cyan]{tml_name}[/bold cyan] from {filename}")
            logging.info(str(tml_name)+' --> ' +
                         str(UploadObject['object'][0]['response']['status']))
        except Exception as e:
            pass
            console.print(
                f"[bold red]failed[/bold red] to recover [bold cyan]{tml_name}[/bold cyan] from {filename} (see log files)")
...
def map_users(cfg_name):
    ...
    r = ts.login(source_username, source_password)
    r.raise_for_status()
    falcon_users = pd.json_normalize(
        ts.metadata_user_list().json(), record_path='headers')
    (
        falcon_users.to_csv(
    ...
    r.raise_for_status()
    file_args = {"sep": "|", "index": False, "encoding": "UTF-8"}
    cloud_users = pd.json_normalize(
        ps.metadata_user_list().json(), record_path='headers')
    (
        cloud_users.to_csv(
...
def map_groups(cfg_name):
    ...
    r = ts.login(source_username, source_password)
    r.raise_for_status()
    falcon_groups = pd.json_normalize(
        ts.metadata_group_list().json(), record_path='headers')
    (
    ...
    r.raise_for_status()
    file_args = {"sep": "|", "index": False, "encoding": "UTF-8"}
    cloud_groups = pd.json_normalize(
        ps.metadata_group_list().json(), record_path='headers')
    (
        cloud_groups.to_csv(
...
def map_users_7(cfg_name):
    ...
    falcon_users.rename(
        columns={"header.id": "id", "header.name": "name"}, inplace=True)
    ps = HTTPClient(ts_url=dest_ts_url)
    r = ps.login(dest_username, dest_password)
    r.raise_for_status()
    file_args = {"sep": "|", "index": False, "encoding": "UTF-8"}
...
def get_cfg(cfg_name):
    ...
    with open('{}.toml'.format(project_config_file), 'r') as file_handler:
        ...
        output_message("Successfully included {}.toml".format(
            project_config_file), "success")
...
def gather_deltas(
    ...
    backup_date: dt.datetime = typer.Option(
        "2012-11-01", help="date of the backup taken"),
):
    """ 
    ...
    user_id_mapping.rename(columns={
                           'id_x': 'id_user_old', 'name': 'user_name', 'id_y': 'id_user_new'}, inplace=True)
    group_id_mapping.rename(columns={
                            'id_x': 'id_group_old', 'name': 'group_name', 'id_y': 'id_group_new'}, inplace=True)
...
def gather_deltas7(
    ...
    df.rename(columns={"header.created": "created",
              "header.modified": "modified"}, inplace=True)
    (df.pipe(comment, msg="Total number of Users: {df.index.size:,}").to_csv(
        data_dir + "/" + "users_all.csv", **file_args))
    ...
    (
        user_df.rename(columns={"header.id": "user_id",
                       "header.name": "user_name"})
        .to_csv(data_dir + "/" + "mapping_users.csv", **file_args)
    )

    user_df.rename(columns={"header.id": "user_id",
                   "header.name": "authorName"}, inplace=True)
    # get high level information about metadata objects
    metadata = {
        # "Users": ts.metadata_user_list,
        # "Groups": ts.metadata_group_list,
...
def create_groups(
    ...
    group_df = pd.read_csv(data_dir + "/" + group_dir +
                           "/" + filename, **file_args)
    ...
    for i in range(len(group_df)):
        try:
            ...
            ts.create_group(TechName=Name, DisplayName=Display,
                            DESC=Description)
...
def migrate_answers(
    ...
    validation_mode: str = typer.Option(...,
                                        help="run in validation mode only True/False"),
    # data_dir: pathlib.Path = typer.Option(..., help="directory to read input data from"),
):
    get_cfg(cfg_name)
    print("Starting Migration of {} answers in validation mode: {}".format(
        migration_mode, validation_mode))
    ...
    for GUID in track(AnswerList, description=f'[green]{mode_type} of {len(AnswerList)} {migration_mode} Answers'):
        ...
        try:
            ...
            print(object_name)
            owner_name = NewAnswersDf.loc[NewAnswersDf['id']
                                          == GUID, 'authorName'].item()
            tml = ts.metadata_tml_export(guid=GUID)
            console.print(
                "Export for " + '{}.answer.tml'.format(object_name) + " successfull", style="info")
            for i in range(0, 10):
                try:
                    ...
                    pass
            UploadObject = ps.metadata_tml_import(
                tml, create_new_on_server=create_new, validate_only=Validate)
            print("Status:")
            console.print(UploadObject['object'][0]
                          ['response']['status'], style="info")
            print("New GUID assigned:")
            print(UploadObject['object'][0]['response']['header']['id_guid'])
            NewObjectsList.append(
                UploadObject['object'][0]['response']['header']['id_guid'])
            Owner = UploadObject['object'][0]['response']['header']['owner_guid']
            OwnerList.append(Owner)
            new_guid = UploadObject['object'][0]['response']['header']['id_guid']
            map_guid.append(
                [object_guid, new_guid, owner_name, object_name, object_type])
            if Validate == True:
                console.print(
                    "Validation for " + '{}.answer.tml'.format(object_name) + " successfull", style="info")
            else:
                console.print(
                    "Imported " + '{}.answer.tml'.format(object_name) + " successfully", style="info")
        except Exception as e:
            ...
            print(object_name)
            # dictionary_data = tml
            map_failed_guid.append(
                [object_guid, owner_name, object_name, object_type])
            # a_file = open(".//failed_answers//{}.Answer.tml_{}".format(object_name,GUID), "wb")
            # pickle.dump(dictionary_data, a_file)
            # a_file.close()
            pass
    df = pd.DataFrame(map_guid, columns=[
                      "old_guid", "new_guid", "owner", "object_name", "object_type"])
    df_failed = pd.DataFrame(map_failed_guid, columns=[
                             "old_guid", "owner", "object_name", "object_type"])
    FailedToLoadAnswer = FailedToLoad
    answer_author = df
    if Validate == False:
        answer_author.to_csv(data_dir + "/info/" +
                             'answer_author.csv', index=False, encoding='UTF8')
        df_failed.to_csv(data_dir + "/info/" +
                         'answer_failed.csv', index=False, encoding='UTF8')
    else:
        df_failed.to_csv(data_dir + "/info/" +
                         'answer_failed.csv', index=False, encoding='UTF8')
        pass

    print("Finished Migration/Validation of " +
          str(len(answer_author)) + " objects")
    if len(FailedToLoadAnswer) == 0:
        console.print(str(len(FailedToLoadAnswer)) +
                      " Errors detected", style="info")
    else:
        console.print(str(len(FailedToLoadAnswer)) +
                      " Errors detected", style="error")
...
def migrate_liveboards(
    ...
    for GUID in track(LiveboardList, description=f'[green]Migration/Validation of {len(LiveboardList)} {migration_mode} Liveboards'):
        ...
        try:
            ...
            tml = ts.metadata_tml_export(guid=GUID)
            print("Export for " +
                  '{}.Liveboard.tml'.format(object_name) + " successfull")
            ...
            try:
                ...
                console.print(UploadObject['object'][0]
                              ['response']['status'], style="info")
                print("New GUID assigned:")
                print(UploadObject['object'][0]
                      ['response']['header']['id_guid'])
                NewObjectsList.append(
                    UploadObject['object'][0]['response']['header']['id_guid'])
                Owner = UploadObject['object'][0]['response']['header']['owner_guid']
                OwnerList.append(Owner)
                ...
                if Validate == True:
                    ...
                    console.print(
                        "Imported " + '{}.Liveboard.tml'.format(object_name) + " successfully", style="info")
            except Exception as e:
                logging.info("couldn't import " + object_name)
                logging.info(str(e))
                error_code = str(e)
                map_failed_guid.append(
                    [object_guid, owner_name, object_name, object_type, error_code])
                pass
            # fix liveboards script run
            pass

        except Exception as e:
            print("couldn't save {}.Liveboard.tml with Guid: ".format(
                object_name, GUID))
    ...
    df = pd.DataFrame(map_guid, columns=[
                      "old_guid", "new_guid", "owner", "object_name", "object_type"])
    df_failed = pd.DataFrame(map_failed_guid, columns=[
                             "old_guid", "owner", "object_name", "object_type", "error_code"])
    # df_failed.to_csv(data_dir +"/"+ "info" + "/"+'failed_liveboards_mapping.csv', index=False,encoding = 'UTF8')
    liveboard_author = df
    if Validate == False:
        liveboard_author.to_csv(data_dir + "/" + "info" + "/" +
                                'liveboard_author.csv', index=False, encoding='UTF8')
        df_failed.to_csv(data_dir + "/" + "info" + "/" +
                         'liveboard_author_failed.csv', index=False, encoding='UTF8')
    else:
        pass
    ...
    console.print(str(len(map_failed_guid)) +
                  " Error(s) detected", style=f'{error_handler}')
    console.print(table)
...
def migrate_worksheets(
    ...
    for GUID in track(WorksheetList, description=f'[green]Migration/Validation of {len(WorksheetList)} {migration_mode} Worksheets'):
        ...
        try:
            ...
            tml = ts.metadata_tml_export(guid=GUID)
            print("Export for " +
                  '{}.Worksheet.tml'.format(object_name) + " successfull")
            UploadObject = ps.metadata_tml_import(
                tml, create_new_on_server=create_new, validate_only=Validate)
            # print("Status:")
            ...
            if Validate == True:
                console.print(
                    "Validation for " + '{}.Worksheet.tml'.format(object_name) + " successfull", style="info")
            else:
                console.print(
                    "Imported " + '{}.Worksheet.tml'.format(object_name) + " successfully", style="info")
        except Exception as e:
            console.print("couldn't save {}.Worksheet.tml".format(
                object_name), style="error")
            FailedToLoad.append(
                [object_guid, owner_name, object_name, object_type])
            print(str(e))
            pass
    ...
    df = pd.DataFrame(map_guid, columns=[
                      "old_guid", "new_guid", "owner", "object_name", "object_type"])
    df_failed = pd.DataFrame(FailedToLoad, columns=[
                             "old_guid", "owner", "object_name", "object_type"])
    worksheet_author = df
    if Validate == False:
        worksheet_author.to_csv(
            data_dir + "/info/" + 'worksheet_author.csv', index=False, encoding='UTF8')
        df_failed.to_csv(data_dir + "/info/" +
                         'worksheet_failed.csv', index=False, encoding='UTF8')
    else:
        df_failed.to_csv(data_dir + "/info/" +
                         'worksheet_failed.csv', index=False, encoding='UTF8')
        pass
    print("Finished Migration/Validation of " +
          str(len(worksheet_author)) + " objects")
    if len(df_failed) == 0:
...
def migrate_tables(
    ...
    table.add_column('Status', justify='center', width=30)
    get_cfg(cfg_name)
    print("Starting Migration of {} tables in validation mode: {}".format(
        migration_mode, validation_mode))
    ts: TSRestApiV1 = TSRestApiV1(server_url=source_ts_url)
    ts.requests_session.verify = False
    try:
    ...
    for GUID in track(tableList, description=f'[green]Migration/Validation of {len(tableList)} {migration_mode} tables'):
        ...
        try:
            object_type = 'LOGICAL_TABLE'
            object_name = NewtablesDf.loc[NewtablesDf['id']
                                          == GUID, 'name'].item()
            owner_name = NewtablesDf.loc[NewtablesDf['id']
                                         == GUID, 'authorName'].item()
            tml = ts.metadata_tml_export(guid=GUID)
            logging.info("Export for " +
                         '{}.table.tml'.format(object_name) + " successfull")
            tbl_tml.loads(json.dumps(tml)).dump(
                f"{data_dir}/tml_export/tables/{migration_mode}/{object_name}_{object_guid}.table.tml")
            UploadObject = ps.metadata_tml_import(
                tml, create_new_on_server=create_new, validate_only=Validate)
            ...
            try:
                ...
                logging.info(
                    "couldn't assign new guid, object already present in target environment")
                new_guid = ''
            ...
            if Validate == True:
                ...
                ps.metadata_assigntag(object_guids=['{}'.format(new_guid)], object_type=[
                                      f'{object_type}'], tag_names=[tag_name])
                logging.info(
                    "Imported " + '{}.table.tml'.format(object_name) + " successfully")
        except Exception as e:
    ...
    table_author = df
    if Validate == False:
        table_author.to_csv(data_dir + "/info/" +
                            'table_author.csv', index=False, encoding='UTF8')
        df_failed.to_csv(data_dir + "/info/" +
                         'table_failed.csv', index=False, encoding='UTF8')
    else:
        pass
    logging.info("Finished Migration/Validation of " +
                 str(len(table_author)) + " objects")
    if (len(df_failed)) > 0:
        error_handler = 'error'
    else:
        error_handler = 'success'
    console.print(str(len(df_failed)) + " Error(s) detected",
                  style=f'{error_handler}')
    console.print(table)
...
def transfer_ownership(
    ...
    """
    output_message(
        "🔑Transferring Ownership for all kind of objects", 'success')
    get_cfg(cfg_name)
    ...
    try:
        ...
        liveboard_author = pd.read_csv(
            data_dir + "/info/" + 'liveboard_author.csv', header=[0])
        print(liveboard_author)
        object_author_table = pd.concat(
            [object_author_table, liveboard_author], ignore_index=True, sort=False)
        console.print(
            "append liveboard authors to object author table", style="info")
    except Exception:
        console.print(
            "WARNING: liveboard author table not present, please migrate liveboards first", style="warning")
...
def add_tags(
    ...
    tag_info = (
        pd.read_csv(data_dir + "/" + "info/" +
                    'answer_author.csv', header=[0], delimiter=',')
    ...
    for i in range(len(tag_info)):
        try:
            ...
            ps.metadata_assigntag(
                ...
                object_type=['QUESTION_ANSWER_BOOK'],
                tag_names=[tag_name])
            console.print('{} Tag assigned to {}'.format(
                tag_name, object_name), style="info")
        except BaseException:
            console.print('failed to assign tag for object: ' +
                          object_name, style="error")
        pass
...
def share_permissions(
    ...
    # Start
    group_map = (
        pd.read_csv(data_dir + "/" + "cs_tools_falcon" + "/" +
                    'ts_group.csv', header=[0], delimiter='|')
        .pipe(comment, msg="Reading group mapping table")
    )
    user_map = (
        pd.read_csv(data_dir + "/" + "cs_tools_falcon" + "/" +
                    'ts_user.csv', header=[0], delimiter='|')
    ...
    new_objects = (
        pd.read_csv(data_dir + "/" + "info" + "/" +
                    'ownership.csv', header=[0], delimiter=',')
        .pipe(comment, msg="Reading object information")
    ...
    new_objects.to_csv(data_dir + "/" + "info" + "/" +
                       "sharing_permissions.txt", index=False, encoding='utf8')
    ...
    for i in range(len(sharing_df)):
        try:
            ...
            perms = {"permissions": {"{}".format(
                user_guid): {"shareMode": "{}".format(sharing_type)}}}
            ts.security_share(shared_object_type=object_type, shared_object_guids=[
                              object_guid], permissions=perms)
            print("shared " +
                  sharing_df['object_name'].iloc[i] + ' with ' + user_guid)
        except BaseException:
