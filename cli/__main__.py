...
def setup_folder_structure(data_dir, cfg_name):
    ...
    # print(
    #     (f"The configuration has been initialised. Please open the file {data_dir}/config/{cfg_name} and",
    #      "make the required changes before continuing."))
...
def create_cfg(
    ...
    data_dir = f"{general_config['BASE_PROJECTS_FOLDER']}/{cfg_name.casefold()}"
...
def get_cfg(cfg_name):
    ...
    project_config_dir = f"{general_config['BASE_PROJECTS_FOLDER']}/{cfg_name.casefold()}/config"
    ...
    with open('{}.toml'.format(project_config_file), 'r') as file_handler:
        ...
        data_dir = f"{general_config['BASE_PROJECTS_FOLDER']}/{cfg_name.casefold()}/output/delta_migrations"
...
def convert_ddls(
    ...
    if Path(general_config.get('DBSCHEMA_STUB')).is_file():
        output = check_output([
            './external_scripts/parse_model.sh',
            str(general_config.get('DBSCHEMA_STUB')),
            str(cfg_name),
...
def gather_deltas(
    ...
    (df.pipe(comment, msg="Total number of Users: {df.index.size:,}").to_csv(data_dir + "/" + "users_all.csv", **file_args))
    ...
    (df.pipe(comment, msg="Total number of Groups: {df.index.size:,}").to_csv(data_dir + "/" + "groups_all.csv", **file_args))
...
def create_groups(
    ...
    group_df = group_df.rename(columns={"header.displayName": "group_displayName", "header.name": "group_name","header.description": "group_description"})
...
def migrate_answers(
    ...
    NewAnswersDf = pd.read_csv(data_dir + "/" + subfolder + "/" + 'answer_{}.csv'.format(migration_mode), header=[0],delimiter='|')
...
def migrate_liveboards(
    ...
    NewLiveboardsDf = pd.read_csv(data_dir + "/" + subfolder + "/" + 'liveboard_{}.csv'.format(migration_mode), header=[0],delimiter='|')
        ...
        try:
            ...
            for i in range(0,100):
                for n in range(0,100):
                    try:
                        if tml['pinboard']['visualizations'][i]['answer']['table']['table_columns'][n]['headline_aggregation'] == 'TABLE_AGGR':
                            tml['pinboard']['visualizations'][i]['answer']['table']['table_columns'][n]['headline_aggregation'] = 'SUM'
                        else:
                            pass
                    except:
                        pass
            try:
                ...
                map_guid.append([object_guid,new_guid ,owner_name,object_name,object_type])
                if Validate == True:
                    console.print("Validation for " + '{}.Liveboard.tml'.format(object_name)+ " successfull",style="info")
                else:
...
def migrate_worksheets(
    ...
    NewWorksheetsDf = pd.read_csv(data_dir + "/" + subfolder + "/" + 'worksheet_{}.csv'.format(migration_mode), header=[0],delimiter='|')
...
def transfer_ownership(
    ...
    for i in range(len(object_author_table)):
        ...
        try:
            ...
            console.print("Transfering Ownership for "+ object_author_table['object_name'].iloc[i]+ " from "+ FromUser + " to " +ToUser,style="info")
        except Exception as e: 
...
def add_tags(
    ...
    tagged_objects = (pd.read_csv(data_dir + "/"+ "cs_tools_falcon" + "/"+'ts_tagged_object.csv', header=[0],delimiter = '|')
            .pipe(comment, msg="ts_tagged_object.csv is present in cs_tools_falcon folder")
    )
    ...
    for i in range(len(tag_info)):
        try:
            ...
            ps.metadata_assigntag(object_guids=['{}'.format(guid)],object_type = ['QUESTION_ANSWER_BOOK'],tag_names=[tag_name])
...
def purge_logfiles(general_config, cfg_name):
    log_folder = f"{general_config['BASE_PROJECTS_FOLDER']}/{cfg_name.casefold()}/log/"
    output_message(f"Purging log files older than {general_config['LOG_FILE_PURGE_SECS']} seconds")
    with os.scandir(log_folder) as listOfEntries:
        for entry in listOfEntries:
            age = time.time() - entry.stat().st_mtime
            if age > general_config['LOG_FILE_PURGE_SECS']:
